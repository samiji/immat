...
$dt = date("Y-m-d");
$verif = mysql_query("select * from immat_coupon where code='" . $code . "' and dt_deb<='" . $dt . "' and dt_fin >='" . $dt . "'")or die(mysql_error());
...
if ($nb > 0) {
    ...
    $verif_code = mysql_query("select * from immat_cmd_detail where idcmd=" . $id_cmd . " and prestation='Coupon de réduction'");
    $nn = mysql_num_rows($verif_code);

    if ($nn == 0) {
        mysql_query("insert into immat_cmd_detail(idcmd,prestation,montant)values(" . $id_cmd . ",'Coupon de réduction','" . $montant . "')")or die(mysql_error());
        $totalpayer = $commande['total'] - $montant;
        mysql_query("update immat_user_info_voiture set total='" . $totalpayer . "' where id=" . $id_cmd)or die(mysql_error());
    } else {
        $totalpayer = $last_amount;
    }
} else {
    echo "<script>alert('Vous avez entrer un code invalide !');</script>";
    $totalpayer = $last_amount;
}
